...
@Component({
  selector: 'clr-compradirecto-agregar',
  styleUrls: ['../compradirecto.psg.scss'],
  templateUrl: './compradirecto-agregar.psg.html',
})
export class CompradirectoAgregarForm implements OnInit {
  ...
  ngOnInit() {
    console.log('Compradirecto agregar()');
  ...
  guardaCompradirecto() {
    ...
    if (this.compradirectoForm.invalid) {
      ...
      this.compradirectoService.postGuardaCompradirecto(this.compradirectoSend).subscribe(
        res => {
          if (res) {

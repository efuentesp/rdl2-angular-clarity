/* PSG  Evaluacionriesgos Edita Ts */
...
import { EvaluacionriesgosSend } from '../evaluacionriesgos.psg.model-send';
...
@Component({
  selector: 'clr-evaluacionriesgos-editar',
  styleUrls: ['../evaluacionriesgos.psg.scss'],
  templateUrl: './evaluacionriesgos-editar.psg.html',
})
export class EvaluacionriesgosEditarForm implements OnInit {
  public evaluacionriesgosForm: FormGroup;
  public submitted = false;
  public loading = false;
  public evaluacionriesgos: Evaluacionriesgos = new Evaluacionriesgos();
  public evaluacionriesgosSend: EvaluacionriesgosSend = new EvaluacionriesgosSend();
  ...
  editaEvaluacionriesgos() {
    this.submitted = true;

    if (this.evaluacionriesgosForm.invalid) {
      swal('Error...', 'Resultados de la Evaluación de Riesgos has fields to fill.', 'error');
    } else {
      this.route.params.subscribe(params => {
        this.idEvaluacionriesgos = params['id'];
      });

      this.evaluacionriesgosSend.campo = this.evaluacionriesgosForm.controls['campo'].value;
      this.evaluacionriesgosSend.fideicomisoId = this.evaluacionriesgosForm.controls['fideicomisoId'].value;
      this.evaluacionriesgosSend.cliente = this.evaluacionriesgosForm.controls['cliente'].value;
      this.evaluacionriesgosSend.estructura = this.evaluacionriesgosForm.controls['estructura'].value;
      this.evaluacionriesgosSend.resultadofinal = this.evaluacionriesgosForm.controls['resultadofinal'].value;

      this.evaluacionriesgosService
        .updateEditaEvaluacionriesgos(this.evaluacionriesgosSend, this.idEvaluacionriesgos)
        .subscribe(res => {
          if (res) {
            if (res.status === 201 || res.status === 200) {
              swal('Success...', 'Resultados de la Evaluación de Riesgos save successfully.', 'success');
              this.location.back();
            } else {
              swal('Error...', 'Resultados de la Evaluación de Riesgos has fields to fill.', 'error');
            }
  ...
  setClickedRowFideicomiso(index, fideicomiso) {
    ...
    if (fideicomiso.checked) {
      ...
      this.evaluacionriesgosForm.controls['fideicomisoId'].setValue(fideicomiso.id);
      this.evaluacionriesgosForm.controls['fideicomisoItem'].setValue(fideicomiso.generalesnumero);
    } else {
      this.fideicomisoService.clear();
      this.evaluacionriesgosForm.controls['fideicomisoId'].setValue(null);
      this.evaluacionriesgosForm.controls['fideicomisoItem'].setValue('');
    }
  }

...
@Component({
  selector: 'clr-kyc-agregar',
  styleUrls: ['../kyc.psg.scss'],
  templateUrl: './kyc-agregar.psg.html',
})
export class KycAgregarForm implements OnInit {
  kycForm: FormGroup;
  submitted = false;
  loading = false;
  ...
  guardaKyc() {
    ...
    if (this.kycForm.invalid) {
      ...
      this.kycService.postGuardaKyc(this.kycSend).subscribe(
        res => {
          if (res) {

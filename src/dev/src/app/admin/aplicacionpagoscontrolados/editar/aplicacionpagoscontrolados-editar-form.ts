/* PSG  Aplicacionpagoscontrolados Edita Ts */
...
import { AplicacionpagoscontroladosSend } from '../aplicacionpagoscontrolados.psg.model-send';
...
@Component({
  selector: 'clr-aplicacionpagoscontrolados-editar',
  styleUrls: ['../aplicacionpagoscontrolados.psg.scss'],
  templateUrl: './aplicacionpagoscontrolados-editar.psg.html',
})
export class AplicacionpagoscontroladosEditarForm implements OnInit {
  public aplicacionpagoscontroladosForm: FormGroup;
  public submitted = false;
  public loading = false;
  public aplicacionpagoscontrolados: Aplicacionpagoscontrolados = new Aplicacionpagoscontrolados();
  public aplicacionpagoscontroladosSend: AplicacionpagoscontroladosSend = new AplicacionpagoscontroladosSend();
  ...
  constructor(
    ...
    this.aplicacionpagoscontroladosForm = this.fb.group({
      ...
      importepago: new FormControl('', Validators.required),
      formapago: new FormControl('', Validators.required),
      tipocomision: new FormControl('', Validators.required),
  ...
  editaAplicacionpagoscontrolados() {
    this.submitted = true;

    if (this.aplicacionpagoscontroladosForm.invalid) {
      swal('Error...', 'Aplicación de pagos controlados has fields to fill.', 'error');
    } else {
      this.route.params.subscribe(params => {
        this.idAplicacionpagoscontrolados = params['id'];
      });

      this.aplicacionpagoscontroladosSend.fideicomisoId = this.aplicacionpagoscontroladosForm.controls[
        'fideicomisoId'
      ].value;
      this.aplicacionpagoscontroladosSend.subfisoId = this.aplicacionpagoscontroladosForm.controls['subfisoId'].value;
      this.aplicacionpagoscontroladosSend.importe = this.aplicacionpagoscontroladosForm.controls['importe'].value;
      this.aplicacionpagoscontroladosSend.importepago = this.aplicacionpagoscontroladosForm.controls[
        'importepago'
      ].value;
      this.aplicacionpagoscontroladosSend.formapago = this.aplicacionpagoscontroladosForm.controls['formapago'].value;
      this.aplicacionpagoscontroladosSend.tipocomision = this.aplicacionpagoscontroladosForm.controls[
        'tipocomision'
      ].value;
      this.aplicacionpagoscontroladosSend.iva = this.aplicacionpagoscontroladosForm.controls['iva'].value;
      this.aplicacionpagoscontroladosSend.moneda = this.aplicacionpagoscontroladosForm.controls['moneda'].value;

      this.aplicacionpagoscontroladosService
        .updateEditaAplicacionpagoscontrolados(this.aplicacionpagoscontroladosSend, this.idAplicacionpagoscontrolados)
        .subscribe(res => {
          if (res) {
            if (res.status === 201 || res.status === 200) {
              swal('Success...', 'Aplicación de pagos controlados save successfully.', 'success');
              this.location.back();
            } else {
              swal('Error...', 'Aplicación de pagos controlados has fields to fill.', 'error');
            }
  ...
  setClickedRowFideicomiso(index, fideicomiso) {
    ...
    if (fideicomiso.checked) {
      ...
      this.aplicacionpagoscontroladosForm.controls['fideicomisoId'].setValue(fideicomiso.id);
      this.aplicacionpagoscontroladosForm.controls['fideicomisoItem'].setValue(fideicomiso.generalesnumero);
    } else {
      this.fideicomisoService.clear();
      this.aplicacionpagoscontroladosForm.controls['fideicomisoId'].setValue(null);
      this.aplicacionpagoscontroladosForm.controls['fideicomisoItem'].setValue('');
    }
  }
  cargaSubfiso() {
  ...
  setClickedRowSubfiso(index, subfiso) {
    ...
    if (subfiso.checked) {
      ...
      this.aplicacionpagoscontroladosForm.controls['subfisoId'].setValue(subfiso.id);
      this.aplicacionpagoscontroladosForm.controls['subfisoItem'].setValue(subfiso.numero);
    } else {
      this.subfisoService.clear();
      this.aplicacionpagoscontroladosForm.controls['subfisoId'].setValue(null);
      this.aplicacionpagoscontroladosForm.controls['subfisoItem'].setValue('');
    }
  }

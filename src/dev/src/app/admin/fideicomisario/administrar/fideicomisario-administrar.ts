/* PSG  Fideicomisario Administrar Ts */
...
@Component({
  selector: 'clr-fideicomisario',
  styleUrls: ['../fideicomisario.psg.scss'],
  templateUrl: './fideicomisario-administrar.psg.html',
})
export class FideicomisarioAdministrar {
  fideicomisarioArray: Fideicomisario[];
  fideicomisario: Fideicomisario;
  idFideicomisario: number;
  loading = false;
  ...
  fideicomisario_update: boolean = false;
  fideicomisario_delete: boolean = false;
  fideicomisario_create: boolean = false;
  fideicomisario_read: boolean = false;
  ...
  ngOnInit() {
    console.log('Fideicomisario administrar()');

    this.getUser();
    this.setButtons();
    this.cargaFideicomisario();

    this.route.params.subscribe(params => {
      this.idFideicomisario = params['id'];
    });

    if (this.idFideicomisario !== undefined) {
      this.getRecuperaFideicomisarioPorFideicomiso(this.idFideicomisario);
    }
  }

  cargaFideicomisario() {
    this.fideicomisarioService.getRecuperaFideicomisario().subscribe(
      res => {
        if (res) {
          if (res.status === 201 || res.status === 200) {
            this.fideicomisarioArray = res.json();
            this.llenaCampos(this.fideicomisarioArray);
          }
        }
      },
      error => {
        swal('Error...', 'An error occurred while calling Fideicomisario.', 'error');
      }
    );
  }

  getRecuperaFideicomisarioPorFideicomiso(id) {
    this.fideicomisarioService.getRecuperaFideicomisarioPorFideicomiso(id).subscribe(
      res => {
        if (res) {
          if (res.status === 201 || res.status === 200) {
            this.fideicomisarioArray = res.json();
      ...
      error => {
        swal('Error...', 'An error occurred while calling the fideicomisario.', 'error');
      }
    );
  }
  ...
  setClickedRowEditaFideicomisario(index, fideicomisario) {
    this.fideicomisarioService.setFideicomisario(fideicomisario);
    if (this.idFideicomisario === undefined) {
      this.router.navigate(['editar', fideicomisario.id], { relativeTo: this.route });
    } else {
      this.router.navigate(['../../editar', fideicomisario.id], { relativeTo: this.route });
    }
  }

  setClickedRowEliminaFideicomisario(index, fideicomisario) {
    this.fideicomisarioService.setFideicomisario(fideicomisario);
    if (this.idFideicomisario === undefined) {
      this.router.navigate(['eliminar', fideicomisario.id], { relativeTo: this.route });
    } else {
      this.router.navigate(['../../eliminar', fideicomisario.id], { relativeTo: this.route });
    }
  }
  ...
  getFideicomisario() {
    if (this.idFideicomisario === undefined) {
      ...
      this.router.navigate(['../../agregar', this.idFideicomisario], { relativeTo: this.route });
    }
  }

/* PSG  Autodeclaracioncrs Agregar Ts */
...
import { AutodeclaracioncrsSend } from '../autodeclaracioncrs.psg.model-send';
...
@Component({
  selector: 'clr-autodeclaracioncrs-agregar',
  styleUrls: ['../autodeclaracioncrs.psg.scss'],
  templateUrl: './autodeclaracioncrs-agregar.psg.html',
})
export class AutodeclaracioncrsAgregarForm implements OnInit {
  autodeclaracioncrsForm: FormGroup;
  submitted = false;
  loading = false;
  public autodeclaracioncrs: Autodeclaracioncrs = new Autodeclaracioncrs();
  public autodeclaracioncrsSend: AutodeclaracioncrsSend = new AutodeclaracioncrsSend();
  public id: number;
  ...
  constructor(
    ...
    this.autodeclaracioncrsForm = this.fb.group({
      ...
      formatocrs: new FormControl('', Validators.required),
    });
  }

  ngOnInit() {
    console.log('Autodeclaracioncrs agregar()');
  ...
  guardaAutodeclaracioncrs() {
    this.submitted = true;

    if (this.autodeclaracioncrsForm.invalid) {
      swal('Error...', 'Autodeclaración CRS has fields to fill.', 'error');
    } else {
      this.autodeclaracioncrsSend.fideicomisoId = this.autodeclaracioncrsForm.controls['fideicomisoId'].value;
      this.autodeclaracioncrsSend.numero = this.autodeclaracioncrsForm.controls['numero'].value;
      this.autodeclaracioncrsSend.nombre = this.autodeclaracioncrsForm.controls['nombre'].value;
      this.autodeclaracioncrsSend.tipopersona = this.autodeclaracioncrsForm.controls['tipopersona'].value;
      this.autodeclaracioncrsSend.tipoparticipante = this.autodeclaracioncrsForm.controls['tipoparticipante'].value;
      this.autodeclaracioncrsSend.formatocrs = this.autodeclaracioncrsForm.controls['formatocrs'].value;

      this.autodeclaracioncrsService.postGuardaAutodeclaracioncrs(this.autodeclaracioncrsSend).subscribe(
        res => {
          if (res) {
            if (res.status === 201 || res.status === 200) {
              swal('Success...', 'Autodeclaración CRS save successfully.', 'success');
              this.location.back();
            } else {
              swal('Error...', 'Autodeclaración CRS has fields to fill.', 'error');
            }
  ...
  setClickedRowFideicomiso(index, fideicomiso) {
    ...
    if (fideicomiso.checked) {
      ...
      this.autodeclaracioncrsForm.controls['fideicomisoId'].setValue(fideicomiso.id);
      this.autodeclaracioncrsForm.controls['fideicomisoItem'].setValue(fideicomiso.generalesnumero);
    } else {
      this.fideicomisoService.clear();
      this.autodeclaracioncrsForm.controls['fideicomisoId'].setValue(null);
      this.autodeclaracioncrsForm.controls['fideicomisoItem'].setValue('');
    }
  }

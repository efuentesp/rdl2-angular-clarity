...
@Component({
  selector: 'clr-tercero-agregar',
  styleUrls: ['../tercero.psg.scss'],
  templateUrl: './tercero-agregar.psg.html',
})
export class TerceroAgregarForm implements OnInit {
  ...
  guardaTercero() {
    ...
    if (this.terceroForm.invalid) {
      ...
      this.terceroService.postGuardaTercero(this.terceroSend).subscribe(
        res => {
          if (res) {

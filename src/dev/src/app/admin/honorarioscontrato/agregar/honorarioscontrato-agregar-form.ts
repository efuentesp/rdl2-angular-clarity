/* PSG  Honorarioscontrato Agregar Ts */
...
import { HonorarioscontratoSend } from '../honorarioscontrato.psg.model-send';
...
@Component({
  selector: 'clr-honorarioscontrato-agregar',
  styleUrls: ['../honorarioscontrato.psg.scss'],
  templateUrl: './honorarioscontrato-agregar.psg.html',
})
export class HonorarioscontratoAgregarForm implements OnInit {
  honorarioscontratoForm: FormGroup;
  submitted = false;
  loading = false;
  public honorarioscontrato: Honorarioscontrato = new Honorarioscontrato();
  public honorarioscontratoSend: HonorarioscontratoSend = new HonorarioscontratoSend();
  public id: number;
  ...
  constructor(
    ...
    this.honorarioscontratoForm = this.fb.group({
      ...
      comisiones: new FormControl('', Validators.required),
      iva: new FormControl('', Validators.required),
    });
  }

  ngOnInit() {
    console.log('Honorarioscontrato agregar()');
  ...
  guardaHonorarioscontrato() {
    this.submitted = true;

    if (this.honorarioscontratoForm.invalid) {
      swal('Error...', 'Cartera honorarios por contrato has fields to fill.', 'error');
    } else {
      this.honorarioscontratoSend.fideicomisoId = this.honorarioscontratoForm.controls['fideicomisoId'].value;
      this.honorarioscontratoSend.subfisoId = this.honorarioscontratoForm.controls['subfisoId'].value;
      this.honorarioscontratoSend.nombre = this.honorarioscontratoForm.controls['nombre'].value;
      this.honorarioscontratoSend.persona = this.honorarioscontratoForm.controls['persona'].value;
      this.honorarioscontratoSend.comisiones = this.honorarioscontratoForm.controls['comisiones'].value;
      this.honorarioscontratoSend.iva = this.honorarioscontratoForm.controls['iva'].value;

      this.honorarioscontratoService.postGuardaHonorarioscontrato(this.honorarioscontratoSend).subscribe(
        res => {
          if (res) {
            if (res.status === 201 || res.status === 200) {
              swal('Success...', 'Cartera honorarios por contrato save successfully.', 'success');
              this.location.back();
            } else {
              swal('Error...', 'Cartera honorarios por contrato has fields to fill.', 'error');
            }
  ...
  setClickedRowFideicomiso(index, fideicomiso) {
    ...
    if (fideicomiso.checked) {
      ...
      this.honorarioscontratoForm.controls['fideicomisoId'].setValue(fideicomiso.id);
      this.honorarioscontratoForm.controls['fideicomisoItem'].setValue(fideicomiso.generalesnumero);
    } else {
      this.fideicomisoService.clear();
      this.honorarioscontratoForm.controls['fideicomisoId'].setValue(null);
      this.honorarioscontratoForm.controls['fideicomisoItem'].setValue('');
    }
  }
  cargaSubfiso() {
  ...
  setClickedRowSubfiso(index, subfiso) {
    ...
    if (subfiso.checked) {
      ...
      this.honorarioscontratoForm.controls['subfisoId'].setValue(subfiso.id);
      this.honorarioscontratoForm.controls['subfisoItem'].setValue(subfiso.numero);
    } else {
      this.subfisoService.clear();
      this.honorarioscontratoForm.controls['subfisoId'].setValue(null);
      this.honorarioscontratoForm.controls['subfisoItem'].setValue('');
    }
  }
